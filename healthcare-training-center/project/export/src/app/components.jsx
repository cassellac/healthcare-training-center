...
/* ---------- Course Card ---------- */
function CourseCard({ course, room, pct, onOpen }) {
  const p = pct == null ? course.pct : pct;
  let action = 'Start', icon = 'ph-circle-dashed', label = 'Not started';
  if (p === 100) { action = 'Review'; icon = 'ph-check-circle'; label = 'Completed'; }
  else if (p > 0) { action = 'Continue'; label = p + '% done'; }
  const onKey = (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onOpen && onOpen(); } };
  return (
    <article className="ln-course" onClick={onOpen} onKeyDown={onKey} role="button" tabIndex={0}>
      <div className="ln-course-thumb" style={{ background: room.color }}>
        <i className={'ph ' + room.icon}></i>
        {p === 100 && <span className="ln-course-done"><i className="ph-bold ph-check"></i></span>}
      </div>
      <div className="ln-course-body">
        ...
        {p > 0 && p < 100 && (
          <div className="ln-course-prog"><ProgressBar pct={p} color={room.color} /></div>
        )}
